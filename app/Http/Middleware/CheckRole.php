        ...
        if (auth()->check()) {
            if (!in_array($route[0], $roleRoutes)) {
                ...
                if ($route[0] != auth()->user()->roles[0]->allowed_route) {
                    $path = $route[0] == auth()->user()->roles[0]->allowed_route ? $route[0].'.show_login_form' : '' . auth()->user()->roles[0]->allowed_route.'frontend.show_login_form';
            ...
            $routeDestination = in_array($route[0], $roleRoutes) ? $route[0].'.show_login_form' : 'user.show_login_form';
